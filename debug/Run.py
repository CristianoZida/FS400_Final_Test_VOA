...
class main_test(QMainWindow, Ui_MainWindow):
    ...
    def get_pd_resp_sig(self,ch):
        ...
        self.ICRtf.write('OUTP1:CHAN1:STATE OFF');time.sleep(0.1)
        self.ICRtf.write( 'OUTP3:CHAN2:POW 5');time.sleep(1)#laser2切换为6dBm 20220704改成5dBm
    ...
    def get_pd_resp_lo(self,ch):
        ...
        #self.ICRtf.write('OUTP1:CHAN1:STATE OFF')
        self.ICRtf.write('OUTP1:CHAN2:STATE OFF');time.sleep(0.1)
        self.ICRtf.write( 'OUTP3:CHAN1:POW 6');time.sleep(1)#laser1切换为9dBm 20220704改成6dBm
...
class WorkThread(QThread):
    ...
    def DC_test(self):
        ...
        *****Start the whole test steps*****
        ...
        if test_result.shape[0]==64:
            print('64通道测试，执行EEPROM写入...')
            gf.write_eeprom_alu(mawin,test_result)
        else:
            print('非64通道测试，不执行EEPROM写入...')
