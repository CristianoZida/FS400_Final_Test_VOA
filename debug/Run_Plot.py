...
import Test_ER_retest as erRetest
...
class main_test(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.setWindowTitle('FS400 Final Test V2.0 - For IDT Driver test')
    ...
    def plotTest(self,df,f,ch,sn,timetmp,typ):
        if typ=='PE':
            ...
            pe_x=df.iloc[0,:][1]
            pe_y=df.iloc[1,:][1]
            new_pahseX=df.iloc[0,:][3:]
            new_pahseY=df.iloc[1,:][3:]
            ...
            ax_px.plot(x_px,df.iloc[:,1],label='Current X')
            ax_px.plot(x_px,df.iloc[:,2],label='Current Y')
            ...
            ax_bw.plot(x_bw,Ysmo.iloc[:,1],label='XI')
            ax_bw.plot(x_bw,Ysmo.iloc[:,2],label='XQ')
            ax_bw.plot(x_bw,Ysmo.iloc[:,3],label='YI')
            ax_bw.plot(x_bw,Ysmo.iloc[:,4],label='YQ')# Plot some data on the axes.
...
class WorkThread(QThread):
    ...
    #plot function
    sig_plot=pyqtSignal(DataFrame,str,str,str,str,str)
    ...
    def plot_test(self):
        ...
        :return:
        '''
        f=r'C:\Users\hp\Desktop\JiawenZhou\Conbine_DC_ICR\FS400_Final_Test_V2.0_0602_ERretest_ICRupdate\Test_report_TXDC\ICR\Normal\WRRC250294_20220601_115121\WRRC250294_CH13_RxBw_FitData_20220601_115121.csv'
        df=pd.read_csv(f)
        self.sig_plot.emit(df,r'C:\Users\hp\Desktop\JiawenZhou\Conbine_DC_ICR','13','test','2022','BW')
        time.sleep(3)
        self.sig_plotClose.emit()
        pass

    #close the figs, only for test
    def fig_close(self):
        plt.close(mawin.fig)
        plt.close(mawin.fig1)
        pass
    ...
    def DC_test(self):
        ...
        *****Start the whole test steps*****
        ...
        for i in range(len(mawin.channel)):
            ...
            PDL=pwr_x-pwr_y
            IL=pwr-float(mawin.ITLA_pwr.iloc[int(mawin.channel[i])-9,1])
            ABC=data[2]
            ER=data[3]
            Tvpi=data[4]
            Max=max_1[:]
            ...
            Min=min_1[:]
            tt=[CH]+[PDL]+[IL]+ABC+ER+Tvpi+Max+Min
    ...
    def er_retest(self,test_result,config,noipwr,er_judgeIQ=15,er_judgeXY=10,di_judge=-3):
        '''
        ...
        while count<retest_flag:
            if len(ch_judge)>0:
                ...
                for i in ch_judge:
                    ...
                    #05252022:add min value to the rusult to record
                    Min=min_1[:]
                    res_retest=config+[CH]+[PDL]+[IL]+ABC+ER+Tvpi+Max+Min
        ...
        *****Start the whole test steps*****
        ...
        for i in range(len(mawin.channel)):
            ...
            if mawin.test_pe:
                ...
                #update06-02-2022:draw fig in main thread
                #fig_X=os.path.join(report_path3,'{}_CH{}_XIXQ_PeSkew_{}.png'.format(sn,ch,timestamp))
                self.sig_plot.emit(pha,report_path3,sn,ch,timestamp,'PE')
