...
class WorkThread(QThread):
    ...
    def VOA_calibration(self):
        ...
        *****Start the whole test steps*****
        ...
        for i in range(len(mawin.channel)):
            ...
            #fre_get = int(re_wl.split('\n')[2].strip().replace('\r', ''), 16)
            list_wl = re_wl.split('\n')
            fre_hex = [list_wl[list_wl.index(ii) + 1].replace('\r', '') for ii in list_wl if 'read_itla' in ii][0]
            fre_get = int(fre_hex.replace('\r', ''), 16)
            cou1 = 0
            while not fre_get == ch_toset:
                print('The {} time set wavelength...'.format(cou1 + 1))
                mawin.CtrlB.write(b'itla_read 0 0x30\n\r')
    ...
    def VOA_calibration_New(self):
        ...
        print('Start DRV EEPROM calibration')
        ...
        if DRV_writeOK:
            ...
            # comlist = ['set_ignore_fs400 1',
            #            'fs400_eep_arg_read 2',
            #            'dbg_set_eep_voa_para 1 -170 4280',
            #            'fs400_eep_arg_save 2',
            #            'set_ignore_fs400 0',
            #            ]
            # # First ignore VOA 1 judgement and revise VOA setting as 0
        ...
        if voa_flag=='1':
            ...
            while not 'g_itla_model' in re_itla:
                ...
                if coun>2:
                    ...
                    # First ignore VOA 1 judgement and revise VOA setting as 1
                    cb.board_set_CmdList(mawin, comlist)
                    return
            s=re_itla.split('\n')
            print(re_itla)
            ...
            *****Start the whole test steps*****
            ...
            for i in range(len(mawin.channel)):
                ...
                while not 'read_itla' in re_wl:
                    ...
                    itla_count+=1
                    if itla_count>10:
                        print('10 times can not read itla wl, reboot')
                        mawin.CtrlB.write(b'reboot 0\r\n')
                        time.sleep(0.2)
                        mawin.CtrlB.write(b'set_ignore_fs400 1\r\n')  # stop VOA judgement
                        time.sleep(0.2)
                        # Check board status:
                        mawin.CtrlB.write(b'rdmsa 0xb016\r\n')
                        time.sleep(0.2)
                        r = mawin.CtrlB.read_all().decode('utf-8', errors='ignore')
                        print(r)
                        count = 0
                        count1 = 0
                ...
                mawin.CtrlB.write(b'show_temperature\n\r');
                time.sleep(0.5)
                re_temp_all=mawin.CtrlB.read_all().decode('utf-8',errors='ignore')
                while not 'module_temperature' in re_temp_all:
                    mawin.CtrlB.write(b'show_temperature\n\r');
                    time.sleep(0.5)
                    re_temp_all = mawin.CtrlB.read_all().decode('utf-8',errors='ignore')
                ...
                fail_count=0
                while not 'voa scan verify success-' in str_wait:
                    str_wait = mawin.CtrlB.readline().decode('utf-8',errors='ignore')
                    print(str_wait)
                    time.sleep(0.1)
                    if 'voa scan verify fail' in str_wait or 'voa_scan_no_dsp complete! low power1' in str_wait or 'voa_scan_no_dsp complete! low power2' in str_wait:
                        print('VOA value got failed for the {} time, try again...'.format(fail_count+1))
                        mawin.CtrlB.write(b'voa_scan_no_dsp 0\n\r');
                        time.sleep(0.2)
                        fail_count+=1

                    if fail_count>4:
                        print('5 times can`t get voa value ,stop the test')
                        ch_jump=True
                        return
                        # count1 = 0
                        # while not 'voa scan verify success-' in str_wait :
